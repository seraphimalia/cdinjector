...
class CDInjector {
  ...
  constructor (chrome, document) {
    ...
    //this.serverAddress = `https://cdn.jsdelivr.net/gh/seraphimalia/cm-eform-scripts@master/src/`
    //this.serverAddress = `https://cdn.jsdelivr.net/gh/seraphimalia/cm-eform-scripts@master/`
    //this.serverAddress = `https://raw.githubusercontent.com/seraphimalia/cm-eform-scripts/master/src/`
    this.serverAddress = `https://cm-eform-scripts.netlify.app/`
    //this.serverAddress = `https://testing.local.munro.co.za:8000/`
    /** @type {Boolean} */
    ...
    if (this.document) {
      ...
      this.iconImage.src = this.chrome.extension.getURL('/cd-16.png')
    }
    ...
    this.chrome.tabs.onActivated.addListener(
      /** @type {{tabId: number}} */ activeInfo =>
        this.updateInterface(activeInfo.tabId)
    )
  }
  ...
  async onScriptRequest (hostName, sender) {
    ...
    await this.loadScript(CDInjector.globalScriptName, 'js', sender)
    await this.loadScript(CDInjector.globalScriptName, 'css', sender)

    for (const domain of CDInjector.iterateDomainLevels(hostName)) {
  ...
  async loadScript (domain, scriptType, sender) {
    ...
    if (scriptContents) {
      scriptContents = await this.processIncludeDirectives(
        scriptContents,
        scriptFileName
      )
  ...
  async processIncludeDirectives (originalScript, originalScriptFileName) {
    ...
    while ((result = includeDirective.exec(originalScript)) !== null) {
      ...
      if (!visitedScripts.has(scriptFileName)) {
        ...
        if (scriptContent) {
          originalScript = CDInjector.spliceString(
            originalScript,
            startIndex,
            endIndex,
            scriptContent
          ...
          originalScript = CDInjector.spliceString(
            ...
            endIndex,
            endIndex,
            ` -- CDINJECTOR: could not include "${scriptFileName}"; script was not found`
          )
        }
      } else {
        ...
        originalScript = CDInjector.spliceString(
          ...
          endIndex,
          endIndex,
          ` -- CDINJECTOR: skipping inclusion of "${scriptFileName}" to avoid dependency cycle`
        )
      }
    }
  ...
  static spliceString (str, startIndex, endIndex, whatToReplaceWith) {
    return (
      str.substring(0, startIndex) + whatToReplaceWith + str.substring(endIndex)
    )
  }
  ...
  updateIconWithScriptCount (count) {
    ...
    this.chrome.browserAction.setIcon({
      imageData: this.iconContext.getImageData(
        0,
        0,
        this.iconSize,
        this.iconSize
  ...
  updateInterface (tabId) {
    ...
    const title = `CDInjector (${count === 0 ? 'no' : countStr} script${
      count === 1 ? '' : 's'
    } loaded)`
...
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
  module.exports = CDInjector // used by Node.js when testing
}
